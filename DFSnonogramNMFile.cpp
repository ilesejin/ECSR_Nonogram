...
#define SYMBOL_EMPTY   "°‡"
// #define SYMBOL_FILLLED "‚óè"
// #define SYMBOL_EMPTY   "‚óã"
...
public:
    ...
    void input() {
        ...
        for (i = 0; i < height; i++) {
            string s;
	        getline(ifs, s);
	        while(s=="") getline(ifs, s);
	        vector<string> VS = split(s, ' ');
	        row_hint_cnt[i] = VS.size();
	        for(int j=0;j<VS.size();j++) {
	            row_hints[i][j] = stoi(VS[j]);
//	            cout << row_hints[i][j] << ' ';
	        }
//	        cout << '\n';
        }
        for (i = 0; i < width; i++) {
	        ...
	        while(s=="") getline(ifs, s);
	        vector<string> VS = split(s, ' ');
	        col_hint_cnt[i] = VS.size();
	        for(int j=0;j<VS.size();j++) {
	            col_hints[i][j] = stoi(VS[j]);
//	            cout << col_hints[i][j] << ' ';
	        }
//	        cout << '\n';
        }
    ...
    static Nonogram load_file(string file_name) {
    	ifs.close();
    	ifs.clear();
        ifs.open(file_name);
        if (!ifs) {
...
//        cin.rdbuf(backup);
    ...
    bool save_file(string file_name) const {
        int i, j;
...
//     ng.print();
//    
	for(int per = 0; per<=100; per+=5) {
		for(int q = 0; q<100; q++) {
			Nonogram ng = Nonogram::load_file("Testcases/Test_6_" + to_string(per) + "/Test_" + to_string(q) + ".txt");
		    st = clock();
			ng.solve();
	//	    ng.print();
		    ng.save_file("DFSResults/Result_6_" + to_string(per) + "/Result_" + to_string(q) + ".txt");
		}
	}
